...
#'
#' @param phase_deg Phase angle psi in degrees (0 = full Moon, 180 = new).
...
sweph_info_to_stars_df = function(sweph_info) {
  ...
  data.frame(
    ...
    ra_deg = sweph_info_df$ra_rad * 180 / pi,
    dec_deg = sweph_info_df$dec_rad * 180 / pi,
...
#'
#' @description Produce a data frame of apparent positions and
...
#' @param elev_m Observer elevation above sea level in metres.
#' @keywords internal
swe_dirs_topo_sunmoon_df = function(datetime, lat, lon, elev_m = 0) {
  swephR::swe_set_topo(lon, lat, elev_m)
  attr(datetime, "tzone") = "UTC"
  ts = as.POSIXlt(datetime, tz = "UTC")
  jd_ut = swephR::swe_utc_to_jd(
  ...
  get_enu_magnitude = function(planet_flag) {
    position_rad = swephR::swe_calc_ut(jd_ut, planet_flag, flg_eq_topo)$xx *
      to_rad
  ...
  info = list(
    sun = get_enu_magnitude(swephR::SE$SUN),
    moon = get_enu_magnitude(swephR::SE$MOON)
  )
  return(sweph_info_to_stars_df(info))
}
...
generate_moon_image_latlong = function(
  ...
  tex = get_moon_texture()

  cot = function(x) {
    1 / tan(x)
  }

  cos_phi = clamp_value(sum(dir_moon * dir_sun), -1, 1)
  phi = acos(cos_phi)

  earth_phase = pi - phi
  earth_phase = clamp_value(earth_phase, 1e-6, pi - 1e-6)
  ...
  if (
    earthshine && is.finite(solar_irradiance_w_m2) && solar_irradiance_w_m2 > 0
  ) {
