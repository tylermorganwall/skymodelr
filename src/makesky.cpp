...
static inline void xyz_to_srgb(double X, double Y, double Z, double &R,
  ...
  const double M[9] = {3.2404542, -1.5371385, -0.4985314, -0.9692660, 1.8760108,
...
static const std::vector<double> &default_lambda_low() {
  static const double lambda_low_arr[] = {360, 380, 400, 420, 440, 460,
                                          480, 500, 520, 540, 560, 580,
                                          600, 620, 640, 660, 680, 700};
  static const std::vector<double> lambda_low(std::begin(lambda_low_arr),
                                              std::end(lambda_low_arr));
  return lambda_low;
}
...
static CIETristimulus interpolate_cie_xyz(double lambda_nm_value) {
  ...
  if (lambda_nm_value < cie_lambda_nm[0] ||
    ...
    Rcpp::stop("lambda_nm entries must be within [%f, %f]", cie_lambda_nm[0],
               cie_lambda_nm[n - 1]);
  }
...
trapezoid_weights(const std::vector<double> &lambda_values) {
...
makesky_rcpp(double albedo = 0.5, double turbidity = 3.0,
  ...
  // deg.
  const double elev_epsilon_deg =
      std::max(0.5 * 180.0 / static_cast<double>(resolution), 1e-4);
  const double safe_elevation =
      clamp_sun_elevation_deg(elevation, elev_epsilon_deg);
  ...
  RcppThread::parallelFor(
      ...
      [&](size_t t) {
        double theta =
            (static_cast<double>(t) + 0.5) / static_cast<double>(nTheta) * M_PI;
        ...
        for (int p = 0; p < nPhi; ++p) {
          double phi = (static_cast<double>(p) + 0.5) / nPhi * 2.0 * M_PI;
          ...
          if (model == "hosek") {
            double gamma = std::acos(clamp_double(v.dot(sun_dir), -1.0, 1.0));
            ...
            Vec3<double> v_zup(std::cos(phi) * std::sin(theta),
                               std::sin(phi) * std::sin(theta),
                               std::cos(theta));
            auto P = prague_model.computeParameters({0.0, 0.0, altitude},
                                                    toPrague(v_zup), elev_rad,
                                                    az_rad, visibility, albedo);
            ...
            for (size_t i = 0; i < N_LAMBDA; ++i) {
              ...
              if (render_sun && view_above_horizon) {
                const double sun =
                    prague_model.sunRadiance(P, lambda_values[i]);
...
double calculate_sun_brightness_rcpp(
    double albedo = 0.5, double turbidity = 3.0, double elevation = 10.0,
    double azimuth_deg = 90.0, std::string model = "hosek",
    std::string prg_dataset = "", double altitude = 0.0,
  ...
  if (model == "hosek") {
    ...
    Vec3<double> sun_dir(std::cos(az_rad) * std::cos(elev_rad),
                         ...
                         std::sin(elev_rad));
    auto P =
        prague_model.computeParameters({0.0, 0.0, altitude}, toPrague(sun_dir),
                                       elev_rad, az_rad, visibility, albedo);
